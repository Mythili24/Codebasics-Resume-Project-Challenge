...
-- query # 7: Get the complete report of the Gross sales amount for the customer “Atliq Exclusive” for each month . 
...
select month(s.date) as month,
       year(s.date) as year,
       (p.gross_price*s.sold_quantity) as gross_sales_amount
...
select *,
case 
	when month(date) >= 1 and month(date) <=3 then 'First'
    when month(date) >= 4 and month(date) <=6 then 'Second'
    when month(date) >= 7 and month(date) <=9 then 'Third'
    when month(date) >= 10 and month(date) <=12 then 'Fourth'
end as Quarter      
from fact_sales_monthly
...
select p.segment,
       ...
       year(s.date) as date,
       s.sold_quantity as sold_quantity,
       (m.manufacturing_cost*s.sold_quantity) as manufacturing_cost
from fact_manufacturing_cost m
inner join fact_sales_monthly s
inner join dim_product p  
where m.product_code = p.product_code
...
select year, sum(manufacturing_cost)
from cte
group by year
;
...
select month, year, sold_quantity, gross_sales_amount
from cte1
group by month, year;